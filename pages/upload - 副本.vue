...
onMounted(() => {
    ...
    try {
        ...
        window.addEventListener("orientationchange", windowOrientationHandler, {
            passive: true,
        });
        deviceOrientationHandler = () => {
            const o =
                typeof window.orientation === "number" ? window.orientation : 0;
            isLandscape.value = Math.abs(Number(o)) === 90;
        };
...
onUnmounted(() => {
    try {
        if (orientationMql && orientationChangeHandler) {
            // @ts-ignore
...
// 镜像翻转：根據當前顯示角度選擇水平或垂直翻轉，確保「左右翻轉」
const toggleMirror = () => {
    if (Croppers.value) {
        const data = Croppers.value.getData();
        const imgData = Croppers.value.getImageData();
        const angle =
            (imgData && imgData.rotate) ||
            (screen.orientation && screen.orientation.angle) ||
            window.orientation ||
            0;
        const rotated90 = Math.abs(Number(angle)) % 180 === 90;
        if (rotated90) {
            const sy = (data && data.scaleY) || 1;
            Croppers.value.scaleY(-sy);
        } else {
            const sx = (data && data.scaleX) || 1;
            Croppers.value.scaleX(-sx);
        }
    }
};
// 确认裁剪
